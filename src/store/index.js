...
export default new Vuex.Store({
    ...
    mutations: {
        ...
        SET_TAGS(state, tags) {
            state.buttonTags0 = tags
        },
        SET_BLOG_ARTICLES20(state, articles) {
            state.blogArticles20 = articles
        },
        SET_FILTER_BLOG_ARTICLES0(state, id) {
            if (state.buttonTags0[id].light == 'tag__batton batton_bacground') {
                state.buttonTags0[id].light = 'tag__batton'
                if (state.filterBlogArticles0.length > state.blogArticles20.length) {
                    state.blogArticles20 = state.filterBlogArticles0.slice(0);
                }
            } else {
                ...
                if (state.filterBlogArticles0.length > state.blogArticles20.length) {
                    state.blogArticles20 = state.filterBlogArticles0.slice(0);
                    state.filterBlogArticles0 = state.blogArticles20.slice(0);
                    state.blogArticles20 = state.blogArticles20.filter((article) => article.tag == state.buttonTags0[id].name)
                } else {
                    state.filterBlogArticles0 = state.blogArticles20.slice(0);
                    state.blogArticles20 = state.blogArticles20.filter((article) => article.tag == state.buttonTags0[id].name)
                }

                for (let i = 0; i < state.buttonTags0.length; i++) {
                    state.buttonTags0[i].light = 'tag__batton'
                }
                state.buttonTags0[id].light = 'tag__batton batton_bacground'
            }
    ...
    actions: {
        ...
        fetchProject({ commit }) {
            const project = [
                ...
                {
                    src: "https://mykaleidoscope.ru/uploads/posts/2020-03/1583855387_38-p-minimalizm-v-dizaine-vannikh-komnat-47.jpg",
        ...
        fetchBlogArticles20({ commit }) {
            ...
            commit("SET_BLOG_ARTICLES20", articles)
        },
    },
    getters: {
        ...
        getTags(state) {
            return state.buttonTags0;
        },
        getBlogArticles20(state) {
            return state.blogArticles20;
        }
    },
    modules: {
    },
